...
export default async function handler(req,res){
  ...
  if(roomId!=="hk-standard") return send(res,400,{error:"Invalid residence."});
  ...
  try{return send(res,200,await getHongKongStayQuote({roomId,checkin,checkout,discountPercent:nights>=7?15:0}));}catch(error){console.error("HK availability failed",error);return send(res,503,{error:"Availability is temporarily unavailable."});}
}
