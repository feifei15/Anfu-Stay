...
export default async function handler(req,res){
  ...
  try{return send(res,200,await getHongKongStayQuote({checkin,checkout}));}catch(error){console.error("HK availability failed",error);return send(res,503,{error:"Availability is temporarily unavailable."});}
}
