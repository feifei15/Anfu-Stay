...
import { getHongKongAirbnbBlockedDates } from "./airbnb-calendar.mjs";
...
async function ensureSchema() {
  if (!schemaReady) schemaReady = (async () => {
    ...
    await sql`ALTER TABLE hk_pricing_overrides ADD COLUMN IF NOT EXISTS source text NOT NULL DEFAULT 'manual'`;
    ...
    await sql`INSERT INTO booking_settings (location, cleaning_fee)
      VALUES ('hk', 39) ON CONFLICT (location) DO NOTHING`;
...
export async function getHongKongStayQuote({ roomId, checkin, checkout, discountPercent = 0 }) {
  ...
  const [rows, airbnbBlockedDates, settings] = await Promise.all([sql`WITH requested AS (
    ...
    ORDER BY requested.stay_date`, getHongKongAirbnbBlockedDates({ start: checkin, end: checkout }),
    sql`SELECT cleaning_fee FROM booking_settings WHERE location='hk'`]);
  ...
  const cleaningFeeUsd = Number(settings[0]?.cleaning_fee ?? 39);
...
export async function getHongKongAdminCalendar(start, end) {
  await ensureSchema(); const sql = sqlClient();
  const [inventory, prices, bookings, weeklyDefaults, airbnbBlockedDates, settings] = await Promise.all([
    ...
    sql`SELECT room_id, stay_date::text, nightly_rate_usd, source FROM hk_pricing_overrides
    ...
    getHongKongAirbnbBlockedDates({ start, end }),
    sql`SELECT cleaning_fee FROM booking_settings WHERE location='hk'`,
  ]);
  const inventoryDates = new Set(inventory.map((row) => row.stay_date));
  for (const stayDate of airbnbBlockedDates) {
    if (!inventoryDates.has(stayDate)) inventory.push({ stay_date: stayDate, status: "airbnb_block", note: "Airbnb reservation" });
  }
  inventory.sort((a, b) => a.stay_date.localeCompare(b.stay_date));
  return { inventory, prices, bookings, weeklyDefaults, cleaningFee: Number(settings[0]?.cleaning_fee ?? 39) };
}

export async function updateHongKongAdminCalendar({ action, start, end, roomId, rateUsd, note, weeklyRates, cleaningFee }) {
  await ensureSchema(); const sql = sqlClient();
  if (action === "set_cleaning_fee") {
    await sql`INSERT INTO booking_settings (location, cleaning_fee) VALUES ('hk', ${cleaningFee})
    ...
    await sql`INSERT INTO hk_pricing_overrides (room_id, stay_date, nightly_rate_usd, source)
      SELECT ${roomId}, day::date, ${rateUsd}, 'manual'
      FROM generate_series(${start}::date, ${end}::date - 1, interval '1 day') day
      ON CONFLICT (room_id, stay_date) DO UPDATE SET
        nightly_rate_usd=EXCLUDED.nightly_rate_usd,source='manual',updated_at=now()`;
...
export async function importHongKongPriceLabsPrices({ roomId = "hk-standard", prices }) {
  await ensureSchema();
  const sql = sqlClient();
  const rows = JSON.stringify(prices.map(({ date, rateUsd }) => ({ date, rateUsd })));
  const result = await sql`WITH incoming AS (
      SELECT value->>'date' AS stay_date, (value->>'rateUsd')::integer AS nightly_rate_usd
      FROM jsonb_array_elements(${rows}::jsonb)
    )
    INSERT INTO hk_pricing_overrides (room_id, stay_date, nightly_rate_usd, source)
    SELECT ${roomId}, stay_date::date, nightly_rate_usd, 'pricelabs' FROM incoming
    ON CONFLICT (room_id, stay_date) DO UPDATE SET
      nightly_rate_usd=EXCLUDED.nightly_rate_usd,source='pricelabs',updated_at=now()
      WHERE hk_pricing_overrides.source='pricelabs'
    RETURNING stay_date`;
