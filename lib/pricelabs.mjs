...
export async function getHongKongPriceLabsPrices({ start, end }) {
  return getPriceLabsPrices({ start, end, listingEnvironment: "PRICELABS_HK_LISTING_ID", pmsEnvironment: "PRICELABS_HK_PMS" });
}

export async function getLasVegasPriceLabsPrices({ start, end }) {
  return getPriceLabsPrices({ start, end, listingEnvironment: "PRICELABS_VEGAS_LISTING_ID", pmsEnvironment: "PRICELABS_VEGAS_PMS" });
}

async function getPriceLabsPrices({ start, end, listingEnvironment, pmsEnvironment }) {
  const apiKey = required("PRICELABS_API_KEY");
  const listingId = required(listingEnvironment);
  const pms = required(pmsEnvironment);
