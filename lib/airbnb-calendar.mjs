...
async function loadCalendar() {
  const url = process.env.AIRBNB_HK_ICAL_URL;
  if (!url) return [];

  const parsed = new URL(url);
  if (parsed.protocol !== "https:" || parsed.hostname !== "www.airbnb.com") {
    throw new Error("AIRBNB_HK_ICAL_URL must be an Airbnb HTTPS calendar URL.");
  }
...
export async function getHongKongAirbnbBlockedDates({ start, end } = {}) {
  ...
  if (!cached || cached.expiresAt <= now) {
    cached = { dates: await loadCalendar(), expiresAt: now + CACHE_TTL_MS };
  }
